...
export default function TopEntidadesChart({ data }: Props) {
  const top = useMemo(() => {
    const acc = new Map<string, number>();
    data.forEach((d) => {
      if (d.labelConsenso === 1 || d.scoreCompuesto > 0.5) {
        if (!d.entidad) return;
        acc.set(d.entidad, (acc.get(d.entidad) ?? 0) + 1);
      }
    });
    return [...acc.entries()]
      .map(([entidad, alertas]) => ({ entidad: shorten(entidad), alertas }))
        ...
        {top.length === 0 ? (
          ...
          <ResponsiveContainer>
            ...
            >
              ...
              />
              <Bar dataKey="alertas" fill="#dc2626" radius={[0, 4, 4, 0]} />
            </BarChart>
          </ResponsiveContainer>
        )}
      </div>
